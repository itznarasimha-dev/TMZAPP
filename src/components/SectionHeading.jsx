    ...
    <div className={className} style={{
      ...
      {eyebrow && (
        <motion.p initial={{ opacity: 0, y: 6 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.35 }}
          className="eyebrow"
          style={{ marginBottom: 12, justifyContent: isLeft ? 'flex-start' : isCenter ? 'center' : 'flex-end' }}
        >{eyebrow}</motion.p>
      )}
      {title && (
      ...
      {description && (
        <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.1 }}
          style={{ marginTop: 12, fontSize: 15, lineHeight: 1.7, color: '#8A92A6', maxWidth: 480, marginLeft: isCenter ? 'auto' : undefined, marginRight: isCenter ? 'auto' : undefined }}
